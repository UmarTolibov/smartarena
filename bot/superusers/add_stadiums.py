...
@bot.message_handler(regexp="🏟️Stadionlarim", is_admin=True)
...
@bot.message_handler(regexp="🌐Stadion qo'shish", state=stadium_sts.init, is_admin=True)
...
@bot.message_handler(content_types=["text"], state=stadium_sts.name, is_admin=True)
...
@bot.message_handler(content_types=["text"], state=stadium_sts.description, is_admin=True)
...
@bot.message_handler(content_types=["photo", "text"], state=stadium_sts.image, is_admin=True)
async def stadium_image_handler(message: Message):
    ...
    if message.content_type == "text" and message.text == "Jo'natib bo'ldim👌":
        await bot.send_message(chat_id, "Stadion narxini yuboring (so'm/soat)", reply_markup=ReplyKeyboardRemove())
        await bot.set_state(user_id, stadium_sts.price, chat_id)


@bot.message_handler(content_types=["text"], state=stadium_sts.price, is_admin=True)
...
@bot.message_handler(content_types=["location"], state=stadium_sts.location, is_admin=True)
