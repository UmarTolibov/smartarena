...
from bot.loader import bot, auth_sts, owner_sts
...
from bot.owners.markups import login_signup


@bot.message_handler(commands=["start"], is_admin=False, is_owner=True)
async def greeting_owner(message: Message):
    ...
    async with Session.begin() as db:
        ...
        if len(user_check) >= 2:
            ...
            await bot.set_state(user_id, owner_sts.menu, chat_id)
            async with bot.retrieve_data(user_id) as data:
                data["user_id"] = user_check[0][1]


@bot.callback_query_handler(func=lambda call: "account" in call.data.split("|"), is_admin=False, is_owner=True)
...
@bot.message_handler(content_types=["text"], state=auth_sts.login_password, is_owner=True, is_admin=False)
async def owner_login_password(message: Message):
    print("ownerlogin")
    user_id = message.from_user.id
    chat_id = message.chat.id
